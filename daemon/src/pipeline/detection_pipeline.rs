...
/// Runs ML inference in blocking thread pool
/// Never blocks frame capture
...
pub async fn run_detection_pipeline(
    ...
    while let Some(raw_frame) = frame_rx.recv().await {
        ...
        let frame = raw_frame.image.clone();  // Arc clone - cheap
        let camera_dimensions = frame.dimensions();  // Original camera frame size
